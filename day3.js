...
db.user.updateOne({passwd:1234},{$set:{userid:'lee',name:'박민지'}})
/*도큐먼트 수정 연산자 
    $set: 필드값을 수정값으로 설정 
...
db.inventory.updateMany({item:'postcard'},{$set:{"dim_cm.$[tagi]":12}},{arrayFilters:[{tagi:10}]})
...
//item이 'journal'인 상품의 tag에서 'blank'를 삭제하세요 
db.inventory.updateOne({item:'journal'},{$pop:{tags:-1}})
db.inventory.updateOne({item:'journal'},{$pop:{tags:1}})

//item이 'paper'인 상품의 tags에서 'blue'를 삭제하세요 
db.inventory.updateMany({item:'paper'},{$pull:{tags:'blue'}})

//item이 'paper'인 상품의 tag에서 'skyblue'를 추가하세요 
...
2. 자유게시판에 아무 글이나 2~3개 작성한다. 특히 그 중에 글 하나에는 
    댓글 하나가 달린 상태로 생성해본다. 댓글은 comment필드에 배열로 넣는다.
3. 비밀게시판을 생성한다.
